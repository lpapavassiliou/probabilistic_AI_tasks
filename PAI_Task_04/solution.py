...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for sizeIdx, _ in enumerate(sizes):
        if sizeIdx < len(sizes) -1:
            modules.append(nn.Linear(sizes[sizeIdx], sizes[sizeIdx+1]))
            if sizeIdx < len(sizes)-2:
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # trajectory's remaining return G_t
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    tdres=self.tdres_buf, logp=self.logp_buf, val=self.val_buf)
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            act = self.get_action(state.numpy())

            v = self.critic.forward(state)
            _, logp = self.actor.forward(state, torch.as_tensor(act))

        return act, v.numpy(), logp.numpy()
    ...
    def get_action(self, obs):
        ...
        distribution, _ = self.actor.forward(torch.as_tensor(obs, dtype=torch.float32))
        act = distribution.sample().numpy()
        return act
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        _, logp = agent.actor.forward(data["obs"], data["act"])
        td_err = data["tdres"]

        loss_actor = - torch.mean(logp * td_err)
        loss_actor.backward()
        ...
        for _ in range(100):
            ...
            values = agent.critic.forward(data["obs"])
            discount_reward_to_go = data["ret"]

            value_criterion  = nn.MSELoss()
            loss_critic = value_criterion(values, discount_reward_to_go)    
