...
def calc_calibration_curve(predicted_probabilities: np.ndarray, labels: np.ndarray, num_bins=30) -> dict:
    ...
    confidences = predicted_probabilities[range(
        num_samples), predicted_classes]
...
class Framework(object):
    ...
    def predict(self, data_loader: torch.utils.data.DataLoader) -> np.ndarray:
        ...
        for batch_x, _ in tqdm.tqdm(data_loader):
            current_probabilities = self.predict_probabilities(
...
class DummyTrainer(Framework):
    ...
    def __init__(self, dataset_train,
        ...
        self.network = MNISTNet(
            in_features=28*28, out_features=10, dropout_p=0.2)
...
class Ensamble2Net(nn.Module):
    def __init__(self,
        ...
        # You could change the depth or width of the model
        self.layer1 = nn.Linear(in_features, hidden_units)
        self.batch1 = nn.BatchNorm1d(hidden_units)
        self.layer2 = nn.Linear(hidden_units, hidden_units)
        self.batch2 = nn.BatchNorm1d(hidden_units)
        self.layer3 = nn.Linear(hidden_units, hidden_units)
        self.batch3 = nn.BatchNorm1d(hidden_units)
        self.layer4 = nn.Linear(hidden_units, out_features)
        self.dropout_p = dropout_p
        self.dropout_at_eval = dropout_at_eval
    ...
    def forward(self, x):
        ...
        # You might add different modules like Pooling
        x = F.dropout(
            F.relu(self.batch1(self.layer1(x))),
            p=self.dropout_p,
            training=self.training or self.dropout_at_eval
        )
        x = F.dropout(
            F.relu(self.batch2(self.layer2(x))),
            p=self.dropout_p,
            training=self.training or self.dropout_at_eval
        )
        x = F.dropout(
            F.relu(self.batch3(self.layer3(x))),
            p=self.dropout_p,
            training=self.training or self.dropout_at_eval
        )
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.network = [Ensamble2Net(in_features=28*28, out_features=10, hidden_units=200) for _ in range(0, self.M)]
        self.train_loader = [torch.utils.data.DataLoader(dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True) for _ in range(0, self.M)]

        self.optimizer = [torch.optim.Adam(self.network[i].parameters(), lr=self.learning_rate) for i in range(0, self.M)]

    def train(self):
        for network, optimizer, train_loader in zip(self.network, self.optimizer, self.train_loader):
            network.train()
            progress_bar = trange(self.num_epochs)
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                    ...
                    # Calculate grad
                    batch_x_clone = Variable(batch_x, requires_grad=True)
                    model_out_x = network(batch_x_clone)

                    loss= F.nll_loss(F.log_softmax(model_out_x, dim=1), batch_y, reduction='mean')
                    loss.backward(retain_graph=True)

                    
                    #batch_xp = batch_x + epsilon * torch.sign(batch_x_clone.grad)
                    #model_out_xp = network(batch_xp)
                    

                    # Calculate the loss
                    # We use the negative log likelihood as the loss
                    # Combining nll_loss with a log_softmax is better for numeric stability
                    # loss = loss_grad + F.nll_loss(F.log_softmax(model_out_xp, dim=1), batch_y, reduction='mean')
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        # batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size,)
        predictions = torch.zeros(x.shape[0], 10)

        for network in self.network:
            network.eval()
            assert x.shape[1] == 28 ** 2
            estimated_probability = F.softmax(network(x), dim=1)
            assert estimated_probability.shape == (x.shape[0], 10)
            predictions += estimated_probability / self.M

            for i in range(x.shape[0]):
                curr_p = predictions[i]
                maximum = torch.max(curr_p)
                arg_max = torch.argmax(curr_p)
                depower = 0.06   # <- set here
                predictions[i, arg_max] = maximum*(1-depower)
                predictions[i] += maximum*depower/10
                predictions[i] /= torch.sum(predictions[i])

                if torch.abs(torch.sum(predictions[i]) - 1) > 0.001:
...
class BackpropTrainer(Framework):
    def __init__(self, dataset_train,  *args, **kwargs):
        ...
        self.network = BayesNet(
            in_features=28 * 28, hidden_features=self.hidden_features, out_features=10)
        self.optimizer = torch.optim.Adam(
    ...
    def predict_probabilities(self, x: torch.Tensor, num_mc_samples: int = 10) -> torch.Tensor:
        ...
        probability_samples = torch.stack(
            [F.softmax(self.network(x)[0], dim=1) for _ in range(num_mc_samples)], dim=0)
...
def evaluate(model: Framework, eval_loader: torch.utils.data.DataLoader, data_dir: str, output_dir: str):
    ...
    if EXTENDED_EVALUATION:
        ...
        reliability_diagram_datax = np.load(os.path.join(
            data_dir, 'reliability_diagram_data_x.npy'))
        reliability_diagram_datay = np.load(os.path.join(
            data_dir, 'reliability_diagram_data_y.npy'))
        for i in range(3):
            ...
            fig.savefig(os.path.join(
                output_dir, str(i)+'_reliability-diagram.pdf'))
